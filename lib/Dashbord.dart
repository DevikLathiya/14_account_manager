...
class _DashbordState extends State<Dashbord> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: () { searchbar(); }, icon:const  Icon(Icons.search)),
        ],
      ),
      drawer:Drawer(width: 270,
        child: SafeArea(
          child: Column(children: [
          Stack(
            children: [
            Container( height: MediaQuery.of(context).size.height*0.40,margin: const EdgeInsets.all(6),
            ...
            Column(children: [
              ...
              Card(margin: const EdgeInsets.all(10),elevation: 5,
                child: Container(decoration: BoxDecoration(color: Colors.grey.shade200,),
                  height: 130,
                  child: Column(children: [
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Padding(padding: EdgeInsets.all(10),child: Text("Credit",style: TextStyle(fontSize: 17),)),
                        Padding(padding: const EdgeInsets.fromLTRB(0, 10, 15, 10),
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      ...
                      Padding(padding: const EdgeInsets.fromLTRB(0, 10, 15, 10),
                      child: Obx(() => Text((_data.mainBalance.value=='') ? "₹00" : "₹ ${_data.mainBalance.value}",style: const TextStyle(fontSize: 17)))),
                    ],)
                  ]),
                ),
              )
            ...
            ListTile(leading: const Icon(Icons.question_answer),title: const Text("Change security Question"),onTap: () => change_question(),),
            ListTile(leading: const Icon(Icons.settings_rounded),title: const Text("Setting"),onTap: () => setting_screen(),),
          ]),
        ),
      ),
      ...
      Obx(() => Container(margin: const EdgeInsets.fromLTRB(0, 10, 0, 20),
        child: ListView.builder(itemCount: _data.getData.length,
          itemBuilder: (context, index) {
            return InkWell(
              ...
              child: Card(margin: const EdgeInsets.symmetric(horizontal: 10,vertical: 5),elevation: 8,
                child: SizedBox(
                  height: height * 0.173,width: double.infinity,
                  child: Column(
                    children: [
                      Row(
                        children: [
                          Container(margin: const EdgeInsets.fromLTRB(10, 0, 0, 5),
                              child: Text("${_data.getData[index]['name']}",style: const TextStyle(fontWeight: FontWeight.w500,fontSize: 20),)),
                          const Spacer(),
                          IconButton(onPressed: () {
                            account_data(_data.getData[index]['id'],_data.getData[index]['name']);
                      ...
                      Row(
                        children: [
                          Container(height: height * 0.095,width: width * 0.27,margin: const EdgeInsets.fromLTRB(9, 7, 5, 7),
                            decoration: BoxDecoration(color: Colors.blue.shade100, borderRadius: BorderRadius.circular(5)),
                            child: Column(mainAxisAlignment: MainAxisAlignment.spaceAround,
                              children: [
                                const Text("Creadit(➹)",style: TextStyle(fontSize: 16,)),
                                Text("₹ ${_data.getData[index]['credit']}",style: const TextStyle(fontSize: 20),)
                              ],
                            ),
                          ),
                          Container(height: height * 0.095,width: width * 0.27,margin: const EdgeInsets.fromLTRB(7, 7, 5, 7),
                            decoration: BoxDecoration(color: Colors.blue.shade100,borderRadius: BorderRadius.circular(5)),
                            child: Column(mainAxisAlignment: MainAxisAlignment.spaceAround,
                              children: [
                                const Text("Debit(➷)",style: TextStyle(fontSize: 16,)),
                                Text("₹ ${_data.getData[index]['debit']}",style: const TextStyle(fontSize: 20),)
                              ],
                            ),
                          ),
                          Container(height: height * 0.095,width: width * 0.27,margin: const EdgeInsets.fromLTRB(7, 7, 5, 7),
                            ...
                            child: Column(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                              children: [
                                ...
                                Text("₹ ${_data.getData[index]['balance']}",style: const TextStyle(fontSize: 20,color: Colors.white),)
                              ],
                            ),
                          ),
                        ],
      ...
      floatingActionButton: FloatingActionButton(
        backgroundColor: Colors.black,tooltip: "Add Account",
        onPressed: () {
        account_data(0,"");
  ...
  changePass(){
    showDialog(context: context, builder: (context) {
      return  AlertDialog(
        title: Container(width: double.infinity,height: 40,color: Colors.cyan,alignment: Alignment.center,
          child: const Text("Change Password?"),),
        content:  Container(height: 165,width: double.infinity,
          child: Column(crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              TextField(controller: _oldpass,keyboardType: TextInputType.phone,
  ...
  searchbar() {
    showDialog(context: context, builder: (context) {
      return AlertDialog(
        title: SizedBox(height: 300,
          child: Column(
            children: [
              Card(margin: EdgeInsets.symmetric(vertical: 10),elevation: 5,
                ...
                itemBuilder: (context, index) {
                  return InkWell(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.person),
                        Container(alignment: Alignment.center,margin: EdgeInsets.all(10),
      ...
      return SimpleDialog(
        ...
        children: [
          Container(height: 300,width: double.infinity,
            child: Column(crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("➽ Change Security questions for retrieve your password when you forgot.",
    ...
    showDialog(context: context, builder: (context) {
      return AlertDialog(
        title: Container(alignment: Alignment.center,
          height: 50,width: 300,color: Colors.cyan,child: const Text("Setting"),
        ),
